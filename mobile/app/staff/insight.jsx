...
import { Bars, Meter, Trend, toneForScore } from '../../src/charts';
import { whatsappHref, telHref, mailHref, open as openLink } from '../../src/contact';
  ...
  return (
    <Screen refreshControl={
      ...
      {!classId ? (
        ...
        <Performance board={board} onOpen={(id) => router.push(`/staff/student/${id}`)} layout={layout} />
      ) : tab === 'attendance' ? (
        <Attendance history={history} onOpen={(id) => router.push(`/staff/student/${id}`)} layout={layout} />
      ) : (
        <Contacts contacts={contacts} school={brand.school?.name} cloud={mode === 'cloud'}
          onOpen={(id) => router.push(`/staff/student/${id}`)} />
      )}
    </Screen>
  );
}
