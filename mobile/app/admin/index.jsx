...
import { AsOf, OfficeScreen, cedis, useOffice } from '../../src/office';
import {
    ...
    <OfficeScreen state={state} skeleton={4}>
      {d ? (
        <>
          ...
          {waiting ? (
            <Card tone="warning" onPress={() => router.push('/admin/approvals')}>
          ...
          {d.fees ? (
            <Section title="Fees" icon="wallet"
              action={<Button label="The office" tone="ghost" size="sm" onPress={() => router.push('/finance')} />}>
              <Grid min={150}>
          ...
          {(d.classes || []).length ? (
            <Section title="How they are doing" icon="trend"
              action={<Button label="In detail" tone="ghost" size="sm" onPress={() => router.push('/admin/academics')} />}>
              <Card padded={false}>
