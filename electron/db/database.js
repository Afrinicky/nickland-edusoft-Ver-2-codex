...
function runMigrations(db) {
  ...
  safe(() => {
    ...
    ins.run('receipt_paper_size', 'roll80');     // A4 | A5 | Letter | roll80 | roll58
  ...
  // 32. Every payment says how it was paid, and carries its own receipt number.
  //
  //     The canteen took money without recording the method or a reference,
  //     because the canteen module only ever took cash across a counter. That
  //     stopped being true the moment a parent could send mobile money for a
  //     month's feeding: a payment with no transaction reference cannot be
  //     checked against anything when the parent says they paid and the school
  //     cannot find it, and that argument is the whole reason the field exists.
  //
  //     Canteen payments also had no receipt number of their own, so the
  //     receipt printed a blank where every other receipt in the system prints
  //     an identifier — which is exactly the piece a parent quotes back.
  //
  //     Additive and idempotent. Existing rows keep their money and get 'Cash',
  //     which is what they were.
  safe(() => {
    const cols = db.prepare("PRAGMA table_info(canteen_payments)").all().map(c => c.name);
    if (!cols.includes('payment_method')) {
      db.exec("ALTER TABLE canteen_payments ADD COLUMN payment_method TEXT DEFAULT 'Cash'");
      db.exec("UPDATE canteen_payments SET payment_method = 'Cash' WHERE payment_method IS NULL");
    }
    if (!cols.includes('reference')) {
      db.exec("ALTER TABLE canteen_payments ADD COLUMN reference TEXT");
    }
    if (!cols.includes('receipt_number')) {
      // Not UNIQUE: SQLite cannot add a unique column to a populated table, and
      // the number is generated by the same shared counter every other receipt
      // uses, which is where uniqueness actually comes from.
      db.exec("ALTER TABLE canteen_payments ADD COLUMN receipt_number TEXT");
      db.exec(`
        UPDATE canteen_payments
           SET receipt_number = 'CT/LEGACY/' || id
         WHERE receipt_number IS NULL
      `);
    }
  });

  // 33. Transport payments carry a reference too, for the same reason.
  safe(() => {
    const cols = db.prepare("PRAGMA table_info(transport_payments)").all().map(c => c.name);
    if (!cols.includes('reference')) {
      db.exec("ALTER TABLE transport_payments ADD COLUMN reference TEXT");
    }
  });

  // 34. Receipts print on an 80mm thermal roll.
  //
  //     The default was A4. A Ghanaian school office collects fees at a counter
  //     with a thermal printer beside it, and an A4 default meant every school
  //     either changed the setting on day one or handed parents a receipt on a
  //     sheet of paper — which is not a receipt, it is a letter.
  //
  //     This DOES move a school already set to A4. There is no way to tell an
  //     A4 that somebody chose from the A4 the installer wrote, and 80mm is the
  //     house format now. A school that wants A4 sets it back under
  //     Settings → Receipt Templates, which is a proprietor-level control.
  //     A5, Letter and 58mm are left exactly as they are: those are choices
  //     nobody could have arrived at by accident.
  safe(() => {
    db.prepare(`
      INSERT INTO settings (key, value, category)
      VALUES ('receipt_paper_size', 'roll80', 'print')
      ON CONFLICT(key) DO UPDATE SET value = 'roll80' WHERE settings.value = 'A4'
    `).run();
  });
