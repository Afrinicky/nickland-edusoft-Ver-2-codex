...
const MODULE_TITLES = {
  ...
  workbook: 'Finance workbook', onboarding: 'Onboarding', session: 'Term and session',
  app: 'The machine itself',
...
lines.push(`| **Hosted \u2014 the installer's own code** | The internet, running host/server.js | ${lanCount} of ${channels.length} (${pct(lanCount)}) |`);
lines.push(`| **Hosted \u2014 the Python service** | The internet, running cloud-python | ${counts.online} of ${channels.length} (${pct(counts.online)}) |`);
lines.push('');
lines.push("There are two ways to be hosted and they are not equal. **host/server.js**");
lines.push("runs the installed application's own handlers on a server \u2014 the same code, so");
lines.push('it answers everything a browser on the school network does, and it cannot');
lines.push('drift, because there is nothing for it to drift from. The Python service is a');
lines.push('separate implementation and answers what has been carried over to it. The');
lines.push('per-channel table below is about that one.');
lines.push('');
