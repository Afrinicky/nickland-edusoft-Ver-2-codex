    ...
    <div className="card no-print">
      <div className="section-header">
        <div>
          ...
          <div className="text-sm text-muted">Editable compilation sheet with Excel import/export: Cls ({sheet?.class_weight || 40}%) + Exam ({sheet?.exam_weight || 60}%) per mapped subject.</div>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button className="btn btn-primary btn-sm" disabled={!classId || !termId || exporting} onClick={exportExcel} title="Export this Assessment Compilation sheet to Excel">📥 {exporting ? 'Exporting…' : 'Export Excel'}</button>
          <button className="btn btn-outline btn-sm" disabled={!classId || !termId || importing} onClick={importExcel} title="Import an edited Assessment Compilation Excel workbook">📤 {importing ? 'Importing…' : 'Import Excel'}</button>
      ...
      <div className="assessment-compilation-actions" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
        <button className="btn btn-primary" disabled={!classId || !termId || exporting} onClick={exportExcel}>📥 {exporting ? 'Exporting…' : 'Export Excel'}</button>
        <button className="btn btn-outline" disabled={!classId || !termId || importing} onClick={importExcel}>📤 {importing ? 'Importing…' : 'Import Excel'}</button>
        <span className="text-sm text-muted" style={{ alignSelf: 'center' }}>Export the selected class/term, edit the workbook, then import it back after validation.</span>
      </div>
    </div>
